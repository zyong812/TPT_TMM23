...
class MOT(CocoDetection):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks,
                 prev_frame=False, prev_frame_range=None, prev_frame_rnd_augs=0.0, norm_transform=None, clip_length=None):
        super(MOT, self).__init__(
            img_folder, ann_file, transforms, return_masks, False,
            norm_transform, prev_frame, prev_frame_rnd_augs, clip_length=clip_length)

        self._prev_frame_range = prev_frame_range

    def sequence_infos(self):
        seqs = self.coco.dataset['sequences']
        frame_names = [img['file_name'].split('_')[0] for img in self.coco.dataset['images']]

        startend_idx = []
        for seq in seqs:
            all_inds = [i for i, n in enumerate(frame_names) if seq==n]
            startend_idx.append((min(all_inds), max(all_inds)))
    ...
    def _add_frame_to_target(self, target, image_id, random_state, key_prefix):
        ...
        # # random jitter ## for debugging, remove aug temporally
        ...
        #
        #     assert(frame_img.size[0] == orig_w and frame_img.size[1] == orig_h)
    ...
    def __getitem__(self, idx):
        ...
        if self._prev_frame:
            ...
            if self.clip_mode:
                # assert self.clip_length >= 2
                sample_interval = np.random.randint(1, self._prev_frame_range+1)
                prev_image_ids = np.sort((frame_id - np.arange(0, frame_id+1, sample_interval))[1:self.clip_length]) + self.coco.imgs[idx]['first_frame_image_id']

                prev_frame_imgs, prev_frame_targets = [], []
                for prev_image_id in prev_image_ids:
                    frame_img, frame_target = self._add_frame_to_target(None, prev_image_id, random_state, 'prev')
            ...
            else: # originally only one previous frame
                prev_frame_id = random.randint(
                    max(0, frame_id - self._prev_frame_range),
                    # min(frame_id + self._prev_frame_range, self.seq_length(idx) - 1)) ## 有可能 prev_frame_id > 当前帧
                    min(frame_id, self.seq_length(idx) - 1))
