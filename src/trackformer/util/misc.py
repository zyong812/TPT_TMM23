...
def collate_fn(batch):
    if isinstance(batch[0][0], list): # clip mode
        assert len(batch) == 1
        batch = list(batch[0])
...
def sttran_collate_fn(batch):
    assert len(batch) == 1 # per video
    image_batch = torch.stack(batch[0][0], dim=0)
    target_list = batch[0][1]
    return image_batch, target_list
