...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 track_query_false_positive_eos_weight, focal_loss, focal_alpha, aux_use_intermediate_match=True):
        ...
        self.aux_use_intermediate_match = aux_use_intermediate_match
        ...
        if self.track_query_false_positive_eos_weight:
            for i, target in enumerate(targets):
                if 'track_query_boxes' in target:
                    ...
                    # ## hardcode: emphasize on detecting new objects
                    # new_obj_query_mask = (targets[i]['track_queries_match_mask'][indices[i][0]] == 0)
                    # new_obj_queries = indices[i][0][new_obj_query_mask]
                    # if len(new_obj_queries) > 1:
                    #     loss_ce[i, new_obj_queries] *= 10
    ...
    def forward(self, outputs, targets):
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                if self.aux_use_intermediate_match: ## else use match of final prediction
        ...
        # for relation losses
        if ('pred_rel_pairs' in outputs_without_aux) and ('pred_relations' in outputs_without_aux):
            ...
            losses.update({
                'loss_relation_proposal': self.relation_proposal_loss(torch.cat(outputs['pred_relation_exists'], dim=0), (all_rel_pair_targets.sum(-1) > 0).float()),
                'loss_relation': self.relation_loss(torch.cat(outputs['pred_relations'], dim=0), all_rel_pair_targets)
            })
            if 'relation_aux_outputs' in outputs:
                for i, aux_outputs in enumerate(outputs['relation_aux_outputs']):
                    losses.update({
                        f'loss_relation_{i}': self.relation_loss(torch.cat(aux_outputs['pred_relations'], dim=0), all_rel_pair_targets)
                    })

        return losses

    def relation_proposal_loss(self, inputs, targets, gamma=2):
        # focal loss to balance positive/negative
        probs = inputs.sigmoid() * 0.9999 # for stability
    ...
    def relation_loss(self, inputs, targets, gamma=2):
        probs = inputs.sigmoid() * 0.9999 + 1e-8 # for stability

        # focal loss to balance hard and easy
        pos_inds = targets.eq(1).float()
        neg_inds = targets.lt(1).float()
        pos_loss = torch.log(probs) * torch.pow(1 - probs, gamma) * pos_inds
        neg_loss = torch.log(1 - probs) * torch.pow(probs, gamma) * neg_inds
