...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, only_match_by_bbox=False):
        ...
        if self.focal_loss:
            ...
            if only_match_by_bbox: # Modification for SGCls mode in SGG task (no given tgt_ids)
                cost_class *= 0
        else:
        ...
        ## propagate prev match to current frame (for training tracking)
        if self.args.tracking_match_propagation:
            cost_min = min(cost_matrix.min(), -1) if cost_matrix.numel() > 0 else -1
            for i, target in enumerate(targets):
                ...
                for j, mask_value in enumerate(target['track_queries_match_mask']):
                    if mask_value.item() == 1:
                        track_query_id = target['track_query_match_ids'][prop_i] # matched box_id
                        prop_i += 1

                        cost_matrix[i, j] = np.inf # img X queries X boxes
                        cost_matrix[i, :, track_query_id + sum(sizes[:i])] = np.inf
                        cost_matrix[i, j, track_query_id + sum(sizes[:i])] = cost_min
        ...
        # ## check match transfer for consistent_pairing
        # for fid, (inds, t) in enumerate(zip(indices, targets)):
        #     qids, obj_ids = inds
        #     if fid > 0:
        #         print('\n')
        #         lost_qids = sorted(list(set(match_dict.keys()) - set(qids)))
        #         for q in lost_qids:
        #             print(f"Lost: qid={q} -> {match_dict[q]}")
        #         for q, o in zip(qids, obj_ids):
        #             if q in match_dict: # tracked obj
        #                 if match_dict[q] == t['track_ids'][o]:
        #                     print(f"Tracked: qid={q} ({match_dict[q]} -> {t['track_ids'][o]})")
        #                 else:
        #                     print(f"!!Match shift: qid={q} ({match_dict[q]} -> {t['track_ids'][o]})")
        #             else:  # queries for new objs
        #                 print(f"New: qid={q} -> {t['track_ids'][o]}")
        #
        #     match_dict = {q: t['track_ids'][o] for q, o in zip(qids, obj_ids)}
