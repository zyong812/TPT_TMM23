...
from ..util.plot_utils import check_prediction
...
class Tracker:
    ...
    def __init__(self, obj_detector, obj_detector_post, tracker_cfg,
        ...
        self.consistent_pairing_detection_thresh = tracker_cfg['consistent_pairing_detection_thresh']
        self.prev_outs = None
    ...
    def add_tracks(self, pos, scores, hs_embeds, masks=None, attention_maps=None, query_ids=None):
        ...
        for i in range(len(pos)):
            ...
            if query_ids is not None:
                self.tracks[-1].query_id = query_ids[i]
    ...
    # step for track_query_propagation_strategy == 'consistent_pairing'
    def consistent_pairing_step(self, blob):
        ...
        ## detection results
        target = None
        num_prev_track = len(self.tracks)
        if num_prev_track > 0:
            target = [self.prev_outs]

        outputs, *_ = self.obj_detector(img, target)
        # check_prediction(img, outputs, frame_id=self.frame_index+1)

        results = self.obj_detector_post['bbox'](outputs, orig_size)
        result = results[0]
        boxes = clip_boxes_to_image(result['boxes'], orig_size[0])

        current_det_scores = result['scores']
        current_det_boxes = boxes
        current_det_keep = torch.logical_and(
            current_det_scores > self.consistent_pairing_detection_thresh,
            result['labels'] == 0) # label is person

        ## TRACKS
        if num_prev_track:
            prev_track_query_ids = set([t.query_id for t in self.tracks])
            current_det_query_ids = set(current_det_keep.nonzero().squeeze(-1).cpu().numpy())

            new_ids = sorted(list(current_det_query_ids - prev_track_query_ids))
            tracked_ids = current_det_query_ids & prev_track_query_ids
            inactive_ids = prev_track_query_ids - current_det_query_ids

            for t in self.tracks:
                if t.query_id in inactive_ids:
                    t.count_inactive += 1
                elif t.query_id in tracked_ids:
                    t.count_inactive = 0
                    t.pos = current_det_boxes[t.query_id]
                    t.score = current_det_scores[t.query_id]

            # remove tracks inactive for long time
            self.tracks = [t for t in self.tracks if t.count_inactive <= self.inactive_patience]

            new_track_ids = []
            if len(new_ids) > 0:
                new_ids_keep = torch.tensor(new_ids)
                new_track_ids = self.add_tracks(
                    current_det_boxes[new_ids_keep],
                    current_det_scores[new_ids_keep],
                    [None] * len(new_ids),
                    query_ids=new_ids)
        else:
            new_track_ids = self.add_tracks(
                current_det_boxes[current_det_keep],
                current_det_scores[current_det_keep],
                [None] * len(current_det_boxes[current_det_keep]),
                query_ids=list(current_det_keep.nonzero().squeeze(-1).cpu().numpy()))

        active_tracks = [t for t in self.tracks if t.count_inactive == 0]
        # NMS to filter duplicates
        if self.detection_nms_thresh and len(active_tracks) > 0:
            track_boxes = torch.stack([t.pos for t in active_tracks])
            track_scores = torch.stack([t.score for t in active_tracks])

            new_track_mask = torch.tensor([
                True if t.id in new_track_ids else False for t in active_tracks])
            track_scores[~new_track_mask] = np.inf
            keep = nms(track_boxes, track_scores, self.detection_nms_thresh)

            remove_track_ids = [track.id for i, track in enumerate(active_tracks) if i not in keep]
            if len(remove_track_ids) > 0:
                self._logger(f'REMOVE TRACK IDS (detection_nms_thresh={self.detection_nms_thresh}): {remove_track_ids}')
            self.tracks = [t for t in self.tracks if t.id not in remove_track_ids]

        ## only record results for active tracks
        track_token_propagation_mask = torch.zeros_like(current_det_scores)
        for track in self.tracks:
            if track.count_inactive > 0: continue
            track_token_propagation_mask[track.query_id] = 1
            ...
            self.results[track.id][self.frame_index]['query_idx'] = track.query_id

        self.prev_outs = {
            'track_query_boxes': outputs['pred_boxes'][0].detach(),
            'track_query_hs_embeds': outputs['hs_embed'][0].to(self.device),
            'track_token_propagation_mask': track_token_propagation_mask
        }
        self.frame_index += 1
    ...
    def step(self, blob):
        ...
        """
        if self.obj_detector.track_query_propagation_strategy == 'consistent_pairing':
            self.consistent_pairing_step(blob)
            return

        ## trackformer tracking on inference
        self._logger(f'FRAME: {self.frame_index + 1}')
        ...
        new_det_keep = torch.logical_and(
            ...
            result['labels'][-self.num_object_queries:] == 0) ## 新检测的目标，只从后300个里面去找
        ...
        # public detection ## self.public_detections=False 时，不会做任何改变
