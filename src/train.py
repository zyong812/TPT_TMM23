...
from trackformer.engine import evaluate, train_one_epoch, evaluate_video_sgg
...
ex.add_named_config('actiongenome', 'cfgs/train_actiongenome.yaml')
...
ex.add_named_config('consistent_pairing', 'cfgs/consistent_pairing.yaml')
ex.add_named_config('hoi', 'cfgs/train_hoi.yaml')
ex.add_named_config('vsgg', 'cfgs/train_vsgg.yaml')
...
def train(args: Namespace) -> None:
    ...
    if args.resume:
        ...
        for k, v in model_state_dict.items():
            if k not in checkpoint_state_dict:
                ...
                elif 'class_embed' or 'semantic_embed' in k:
    ...
    param_dicts = [
        {"params": [p for n, p in model_without_ddp.named_parameters()
                    if not match_name_keywords(n, args.lr_backbone_names + args.lr_linear_proj_names + ['layers_track_attention'] + scratch_params) and p.requires_grad],
         "lr": args.lr,},
        {"params": [p for n, p in model_without_ddp.named_parameters()
        ...
        {"params": [p for n, p in model_without_ddp.named_parameters()
                    if match_name_keywords(n, scratch_params) and p.requires_grad],
         "lr": args.lr * 10},
    ]
    print(f'scratch_params={scratch_params} with lr={args.lr * 10}')
    ...
    # # check data
    # imgs, tgts = data_loader_train.dataset.__getitem__(417)
    # for j in range(len(tgts)): check_annotation(nested_tensor_from_tensor_list(imgs), tgts, idx=j)
    ...
    if args.eval_only:
        if args.video_sgg_eval:
            evaluate_video_sgg(model, data_loader_val.dataset, device, args, postprocessors)
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if args.output_dir:
            if args.save_model_interval and not epoch % args.save_model_interval:
                checkpoint_paths.append(output_dir / f"checkpoint_epoch{epoch}.pth")
